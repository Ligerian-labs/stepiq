...
export const linearAdapter: ConnectorProviderAdapter = {
  provider: "linear",
  getCapabilities() {
    return capabilities;
  },
  buildPayload(args: BuildPayloadArgs) {
    if (!isSupportedAction(args.capability_id)) {
      throw new Error(`Unsupported capability \"${args.capability_id}\" for provider \"linear\"`);
    }
    return { ...args.input };
  },
  async callTool(capability_id: string, payload: Record<string, unknown>, _context: ProviderCallContext) {
    if (!isSupportedAction(capability_id)) {
      throw new Error(`Unsupported capability \"${capability_id}\" for provider \"linear\"`);
    }
    return {
      kind: "action",
      result: makeActionResult("linear", capability_id, {
