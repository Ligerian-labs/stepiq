...
const maxTurnCompletionDuration = 60 * time.Second
...
func runAgentStep(raw string) (*AgentOutput, error) {
	...
	for turn := 1; turn <= in.Agent.MaxTurns; turn++ {
		turnBudget := maxTurnCompletionDuration
		...
		emitAgentLog("info", "agent_turn_started", "Agent turn started", map[string]any{
			...
			"turn_timeout_ms": turnBudget.Milliseconds(),
		})
		...
		turnCtx, cancelTurn := context.WithTimeout(ctx, turnBudget)
		resp, err := provider.Completion(turnCtx, params)
		cancelTurn()
		if err != nil {
		...
		for _, tc := range choice.Message.ToolCalls {
			...
			toolResult, callErr := callToolHost(tc.Function.Name, tc.Function.Arguments, in.Agent.NetworkAllowlist)
			if callErr != nil {
...
func callToolHost(name, args string, allowlist []string) (string, error) {
	...
	then := js.FuncOf(func(this js.Value, args []js.Value) any {
		...
		if len(args) > 0 {
			callErr = errors.New(args[0].String())
		} else {
